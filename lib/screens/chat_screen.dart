...
class _ChatScreenState extends State<ChatScreen> with TickerProviderStateMixin {
  ...
  void _scrollListener() {
    ...
    if (_scrollController.position.pixels ==
        ...
        !_isLoadingMore &&
        _messages.length >= 20) {
      // Only enable pagination if we have enough messages
      _loadMoreMessages();
    }
  }
  ...
  Future<void> _loadMessages() async {
    ...
    try {
      ...
      if (mounted && _messages.isEmpty) {
        setState(() {
          // Keep existing state but show loading
        });
      }
      ...
      if (_chatService != null) {
        ...
        if (mounted) {
          ...
          // Scroll to bottom for new conversations or to last unread message
  ...
  void _showErrorSnackBar(String message) {
    // Use the error banner widget for better UX
    if (mounted) {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => FileViewerScreen(
          fileName: fileName,
          fileType: fileType,
          fileUrl: 'https://example.com/files/$fileName', // Placeholder URL
        ),
      ),
    );
  }
            ...
            children: [
              if (_isMultiSelectMode) ...[
                ...
                ListTile(
                  ...
                  onTap: () {
                    Navigator.pop(context);
                    // Navigate to contact info screen with specific contact info
                    if (_selectedRecipientId != null &&
                        _selectedRecipientName != null) {
                      // Pass specific contact info if available
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                      ...
                      // Fall back to general team info
                      Navigator.push(
                        context,
                        MaterialPageRoute(
